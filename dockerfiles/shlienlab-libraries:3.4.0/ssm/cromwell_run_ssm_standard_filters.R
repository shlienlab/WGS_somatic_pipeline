...
cromwell_get.mutect2.data <- function(annovar_file, load_vcfs = TRUE, vcf_file = "", t_gt = "TUMOR", n_gt = "NORMAL") {
  ...
  if (load_vcfs) {
    ...
    chr_vcf_data <- try(vcfR::vcfR2tidy(vcfR::read.vcfR(vcf_file, verbose=FALSE), verbose=FALSE, single_frame=TRUE))
    if (class(chr_vcf_data) == 'try-error') {
      # If any VCF files are missing, then we don't load any of them.
      stop("Error reading VCF file.")
    }
    else {
      t <- chr_vcf_data$dat %>% filter(Indiv == t_gt) %>% select(CHROM, POS, REF, ALT, starts_with("gt")) 
      g <- chr_vcf_data$dat %>% filter(Indiv == n_gt) %>% select(CHROM, POS, REF, ALT, starts_with("gt")) 
      
      # Clean up each of these dataframes
      for (gt in c('g', 't')) {        
        assign(gt, get(gt) %>%
          dplyr::select(-gt_GT_alleles) %>% 
          dplyr::rename_with(~gsub("^gt_", "", .x, perl = TRUE)) %>%
          dplyr::rename(gt_POS = POS, 
                        gt_REF = REF, 
                        gt_ALT = ALT)
        )
      }
      vcf_data <- full_join(t, g, 
          by=c("CHROM", "gt_POS", "gt_REF", "gt_ALT"), 
          suffix = c("_t", "_n")
          ) %>% dplyr::select(-CHROM) 
...
write_delim(annots, file=annot_file, delim="\t")
