...
merge.txt <- function(files) {
  ...
  for (f in files) {
    data <- try(
      read_tsv(file = f, col_names = TRUE, col_types = cols(.default = "?", annovar_chr = col_factor(levels = intervals), annovar_ref = "c", annovar_alt = "c", annovar_esp = "d", g_REF = "c", g_ALT = "c", t_REF = "c", t_ALT = "c", in_centromere = "c", normal_coverage_threshold = "c", unique_mapping = "c", high_depth = "c", dist_low_complexity = "c", multi_mapping = "c")),
      silent = TRUE
    )
...
snv_data <- merge.txt(snvs)
indel_data <- merge.txt(indels)
